...
void cb_basic_cancel(myuvamqp_channel_t *channel, myuvamqp_reply_t *reply, void *arg);
void do_basic_cancel(void *arg);
...
int main(int argc, char *argv[])
{
    ...
    ASSERT(1 == cb_basic_cancel_called);
...
void cb_basic_consume(myuvamqp_channel_t *channel, myuvamqp_reply_t *reply, void *arg)
{
    ...
    myuvamqp_channel_free_reply(reply);
    call_later(UV_LOOP(channel->connection), 500, do_basic_cancel, channel);
}
...
void do_basic_cancel(void *arg)
{
    ...
    err = myuvamqp_channel_basic_cancel(channel, consumer_tag, FALSE, cb_basic_cancel, channel);
    ASSERT(0 == err);
}
...
{
    int err;
    myuvamqp_basic_cancel_reply_t *basic_cancel_reply = (myuvamqp_basic_cancel_reply_t *) reply;

    cb_basic_cancel_called += 1;

    ASSERT(arg == channel);
    ASSERT(MYUVAMQP_BASIC_CLASS == basic_cancel_reply->class_id);
    ASSERT(MYUVAMQP_BASIC_CANCEL == basic_cancel_reply->method_id);
    ASSERT(MYUVAMQP_REPLY_CODE_SUCCESS == basic_cancel_reply->reply_code);
    ASSERT(NULL == basic_cancel_reply->reply_text);
    ASSERT(0 == strcmp(consumer_tag, basic_cancel_reply->consumer_tag));
    ASSERT(1 == channel->channel_id);
    ASSERT(0 == MYUVAMQP_LIST_LEN(channel->operations[MYUVAMQP_BASIC_CANCEL_OPERATION]));
