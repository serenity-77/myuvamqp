...
void cb_basic_qos(myuvamqp_channel_t *channel, myuvamqp_reply_t *reply, void *arg);

int connect_cb_called = 0;
int auth_cb_called = 0;
int cb_channel_opened_called = 0;
int cb_basic_qos_called = 0;
...
void cb_channel_opened(myuvamqp_channel_t *channel, myuvamqp_reply_t *reply, void *arg)
{
    ...
    err = myuvamqp_channel_basic_qos(channel, 0, 10, FALSE, cb_basic_qos, channel);
    ASSERT(0 == err);
}

void cb_basic_qos(myuvamqp_channel_t *channel, myuvamqp_reply_t *reply, void *arg)
{
    myuvamqp_basic_qos_reply_t *basic_qos_reply = (myuvamqp_basic_qos_reply_t *) reply;

    cb_basic_qos_called += 1;

    ASSERT(arg == channel);
    ASSERT(MYUVAMQP_BASIC_CLASS == basic_qos_reply->class_id);
    ASSERT(MYUVAMQP_BASIC_QOS == basic_qos_reply->method_id);
    ASSERT(MYUVAMQP_REPLY_CODE_SUCCESS == basic_qos_reply->reply_code);
    ASSERT(NULL == basic_qos_reply->reply_text);
    ASSERT(1 == channel->channel_id);
    ASSERT(0 == MYUVAMQP_LIST_LEN(channel->operations[MYUVAMQP_BASIC_QOS_OPERATION]));
