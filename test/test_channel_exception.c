...
void cb_queue_declared(myuvamqp_channel_t *channel, myuvamqp_reply_t *reply, void *arg)
{
    myuvamqp_connection_t *amqp_conn = channel->connection;
    myuvamqp_queue_declare_reply_t *queue_declare_reply = (myuvamqp_queue_declare_reply_t *) reply;

    cb_queue_declared_called += 1;

    ASSERT(arg == channel);
    ASSERT(MYUVAMQP_REPLY_CODE_NOT_FOUND == queue_declare_reply->reply_code);
    ASSERT(0 == strcmp("NOT_FOUND - no queue 'myuvamqp-test-queue' in vhost '/'", queue_declare_reply->reply_text));
    ASSERT(MYUVAMQP_QUEUE_CLASS == queue_declare_reply->class_id);
    ASSERT(MYUVAMQP_QUEUE_DECLARE == queue_declare_reply->method_id);
    ASSERT(NULL == queue_declare_reply->queue);
    ...
    myuvamqp_connection_remove_channel(amqp_conn, channel);
    ASSERT(1 == MYUVAMQP_LIST_LEN(amqp_conn->channels));
    myuvamqp_channel_free_reply(reply);
