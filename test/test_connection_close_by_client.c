...
void cb_disconnect(myuvamqp_connection_t *amqp_conn, void *arg);
void do_disconnect(uv_timer_t *timer);
...
void cb_channel_opened(myuvamqp_channel_t *channel, myuvamqp_reply_t *reply, void *arg)
{
    ...
    {
        disconnect_timer_t *disconnect_timer = myuvamqp_mem_alloc(sizeof(*disconnect_timer));
        disconnect_timer->connection = channel->connection;
        uv_timer_init(((uv_handle_t *) channel->connection)->loop, (uv_timer_t *) disconnect_timer);
        uv_timer_start((uv_timer_t *) disconnect_timer, do_disconnect, 500, 0);
    }
}

void cb_disconnect(myuvamqp_connection_t *amqp_conn, void *arg)
{
    cb_disconnect_called += 1;
    ASSERT(NULL == arg);
    ASSERT(MYUVAMQP_STATE_NOT_CONNECTED == amqp_conn->connection_state);
    ASSERT(NULL == amqp_conn->channels);
    ASSERT(0 == amqp_conn->channel_ids);
}
...
void do_disconnect(uv_timer_t *timer)
{
    disconnect_timer_t *disconnect_timer = (disconnect_timer_t *) timer;
    myuvamqp_connection_t *amqp_conn = disconnect_timer->connection;

    uv_close((uv_handle_t *) timer, cb_timer_closed);

    myuvamqp_connection_set_close_callback(amqp_conn, cb_disconnect, NULL);

    myuvamqp_connection_close(amqp_conn);
    ASSERT(MYUVAMQP_STATE_CLOSING == amqp_conn->connection_state);
}
