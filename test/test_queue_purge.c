...
void cb_queue_purged(myuvamqp_channel_t *channel, myuvamqp_reply_t *reply, void *arg);
void do_queue_purge(void *arg);
...
int main(int argc, char *argv[])
{
    ...
    ASSERT(1 == cb_queue_purged_called);
...
void cb_queue_declared(myuvamqp_channel_t *channel, myuvamqp_reply_t *reply, void *arg)
{
    ...
    call_later(UV_LOOP(channel->connection), 500, do_queue_purge, channel);
}
...
{
    int err;
    myuvamqp_queue_purge_reply_t *queue_purge_reply = (myuvamqp_queue_purge_reply_t *) reply;

    cb_queue_purged_called += 1;

    ASSERT(arg == channel);
    ASSERT(MYUVAMQP_QUEUE_CLASS == queue_purge_reply->class_id);
    ASSERT(MYUVAMQP_QUEUE_PURGE == queue_purge_reply->method_id);
    ASSERT(MYUVAMQP_REPLY_CODE_SUCCESS == queue_purge_reply->reply_code);
    ASSERT(NULL == queue_purge_reply->reply_text);
    ASSERT(0 == queue_purge_reply->message_count);
    ASSERT(1 == channel->channel_id);
    ASSERT(0 == MYUVAMQP_LIST_LEN(channel->operations[MYUVAMQP_QUEUE_PURGE_OPERATION]));
...
void do_queue_purge(void *arg)
{
    ...
    err = myuvamqp_channel_queue_purge(channel, queue_name, FALSE, cb_queue_purged, channel);
    ASSERT(0 == err);
}
